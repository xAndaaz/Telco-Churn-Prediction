...
if st.session_state['page'] == 'Batch Analysis':
    ...
    if st.session_state['batch_results_df'] is not None:
        ...
        with st.container(border=True):
            ...
            st.markdown("""
            The **Actionable Insight** column is the core of our advisory engine. It's generated by a sophisticated process that combines multiple advanced techniques:
            - **Classification Model:** An XGBRFClassifier predicts the *likelihood* of churn.
            - **Explainable AI (XAI):** We use SHAP to identify the top 5 key drivers (*why*) for each individual's prediction.
            - **Survival Analysis:** A Cox Proportional Hazards model predicts the *urgency* of churn (*when*).
            - **Customer Lifetime Value (CLV):** Customers are segmented into value tiers to help prioritize retention efforts.
            
            Our context-aware engine intelligently synthesizes these elements into a clear, human-readable recommendation, ensuring the advice is factually consistent with the customer's data.
            """)
            display_columns = ['customerID', 'churn_prediction', 'ProbabilityRiskTier', 'TimeBasedRisk', 'ActionableInsight', 'clv_tier']
